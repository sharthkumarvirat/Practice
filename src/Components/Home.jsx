...
export default function Home() {
  ...
  const handleSubmit = () => {
    if (name !== "", age !== "", salary !== "") {
      axios.post("http://localhost:4000/profile", { name, age, salary })
        .then(() => { setName(""); setAge(""); setSalary(""); })
    ...
    <div className=' flex items-center w-full h-full justify-around mt-20'>
      <div className='w-full h-full flex  flex-col items-center justify-between gap-2'>
        ...
        <label className="input input-bordered flex items-center gap-2">
          ...
          <input type="Salary" className="grow" value={salary} onChange={(e) => { setSalary(e.target.value) }} />
        </label>
